...
void equilibrium(const int *arr, int size)
{
    int i;
    int R = 0;
    int L = 0;
    ...
    for (i=0; i<size; i++) {
        ...
        if (i == 0) {
            ...
            /**
             * Left side gets the last but one element (when last but one convention applies,
             * i.e doesn't apply for 0 index). Right side is just Sum - Current Left and the element that
             * we are on.
             */
            L += *(arr + i-1);
            R = (S - (L + *(arr + i)));
 ...
 int main()
 {
     ...
     equilibrium(arr_null, sizeof(int));
     return 0;
 }
